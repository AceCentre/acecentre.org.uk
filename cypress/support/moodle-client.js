...
function client(options) {
  ...
  if ("token" in options) {
    ...
    self.logger.debug(
      "[init] no explicit token provided - requires authentication"
    );
  }
...
function authenticate_client(client, username, password) {
  return new Promise(function (resolve, reject) {
    client.logger.debug(
      "[init] requesting %s token from %s",
      client.service,
      client.wwwroot
    );
    var options = {
    ...
    request_promise(options)
      .then(function (res) {
        if ("token" in res) {
          ...
          client.logger.error(
            "[init] authentication failed: unexpected server response"
          );
          reject(
            new Error("authentication failed: unexpected server response")
          );
        }
      })
      .catch(function (err) {
