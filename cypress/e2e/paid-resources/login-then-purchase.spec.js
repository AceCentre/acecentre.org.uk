...
context("Paid resources", () => {
  ...
  afterEach(async () => {
    cy.wrap(null, { timeout: 60000 }).then(async () => {
      await deleteUser(newEmail, "digitalocean", "login-then-purchase");
    });
  });

  it.skip("Add a product to cart, login at checkout and then purchase item already in cart", () => {
    // Register for a new account
    newEmail = validEmail("login-then-purchase");
    cy.visit("");
