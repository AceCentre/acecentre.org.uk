...
context("Paid resources", () => {
  ...
  afterEach(async () => {
    cy.wrap(null, { timeout: 60000 }).then(async () => {
      await deleteUser(newEmail, "digitalocean", "new-account-3d-auth");
    });
  });

  it.skip("can add product to cart, and checkout with a new account, with 3D Auth", () => {
    newEmail = validEmail("new-account-3d-auth");
