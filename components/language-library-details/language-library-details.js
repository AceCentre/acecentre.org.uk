...
const uniqueValues = (key) => (resources, results) => {
  let byKey = {};

  for (let resource of resources) {
    for (let current of resource[key].nodes) {
      byKey[current.slug] = {
        slug: current.slug,
        name: current.name,
        count: 0,
      ...
      sections: [
        {
          ...
          getAllUniqueValues: uniqueValues("languages"),
          getDetail: (resource) =>
            resource.languages.nodes.map((x) => x.name).join(", "),
          getFilterValues: (resource) =>
            resource.languages.nodes.map((x) => x.slug),
        },
        {
