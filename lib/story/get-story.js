...
export const getSimpleStoryByIndex = async (index) => {
  if (index === undefined) {
    return null;
  }
  ...
  try {
    ...
    if (!caseStudies.nodes) return null;

    const current = caseStudies.nodes[index] || null;

    return nodeToSimpleStory(current);
  } catch (error) {
    console.error("GraphQL request failed for story by index:", error);
    return null;
  }
};
...
export const getSimpleStory = async (slug) => {
  ...
  try {
    ...
    console.error("GraphQL request failed for simple story:", error);
    return null;
  }
};
...
export const getAllStories = async () => {
  try {
    ...
    console.error("GraphQL request failed for all stories:", error);
    return [];
  }
};
