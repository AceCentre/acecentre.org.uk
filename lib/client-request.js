...
export const clientRequest = async (req, res, graphQlQuery, variables) => {
  ...
  return makeRequest(req, res, graphQlQuery, variables);
};

const makeRequest = async (req, res, graphQlQuery, variables) => {
...
const refreshWooToken = async (req, res) => {
  const cart = req.session.get("cart") || {};
  ...
  await req.session.save();
  await makeRequest(req, res, GET_CART);
};
