...
export const clientRequest = async (req, graphQlQuery, variables, extraHeaders = {}) => {
  ...
  return makeRequest(req, graphQlQuery, variables, extraHeaders);
};

const makeRequest = async (req, graphQlQuery, variables, extraHeaders = {}) => {
  const user = req.session.get("user") || {};
  const cart = req.session.get("cart") || {};
  const authToken = user.authToken || null;
