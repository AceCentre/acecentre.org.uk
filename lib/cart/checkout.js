...
export async function updateCustomer(req, res, body) {
  const input = bodyToInput(body);
  await clientRequest(req, res, UPDATE_CUSTOMER, {
    billing: input.billing,
  });
}

export async function checkout(req, res, body, cohortNames) {
  const input = bodyToInput(body, cohortNames);

  const response = await clientRequest(req, res, CHECKOUT, {
    input,
  });
