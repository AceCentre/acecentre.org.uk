...
export const getEmployee = async (slug) => {
  ...
  try {
    ...
    console.error("GraphQL request failed for employee:", error);
    return null;
  }
};
...
export const getAllStaff = async () => {
  try {
    ...
    console.error("GraphQL request failed for staff:", error);
...
const getGroups = (node) => {
  if (!node.staffTypes || !node.staffTypes.nodes) {
    return [];
  }
...
const getLocation = (node) => {
  ...
  if (!node.staffLocations || !node.staffLocations.nodes) {
    return location;
  }
