...
export const getOrders = async (req, user) => {
  ...
  return response.customer.orders.nodes.map((order) => {
    const lines = order.lineItems.nodes.map((line) => {
      const moodleProduct = isMoodleProduct(line.product.node);
      ...
      if (
        line &&
        line.product.node &&
        line.product.node.__typename === "BundleProduct"
      ) {
        type = "Bundle";
        urlPrefix = "learning";
      }
      ...
      return {
        name: line.product?.node.name || "Unknown",
        quantity: line.quantity,
        total: line.total == null ? "Free" : `£${line.total}`,
        type,
        href: `/${urlPrefix}/${line.product.node?.slug || "all"}`,
      };
    });
