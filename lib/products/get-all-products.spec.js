...
it("logs a warning and returns products if there are exactly 200 products (fetch limit)", async () => {
  // Arrange
  let nodes = [];
  for (let i = 0; i < 200; i++) {
    nodes.push({
  ...
  // Assert
  // Should return products instead of throwing
  expect(result).toBeDefined();
  expect(Array.isArray(result)).toBe(true);
  // Should log a warning about hitting the limit
  expect(consoleSpy).toHaveBeenCalledWith(
    expect.stringContaining("Hit 200 unique products")
  );
