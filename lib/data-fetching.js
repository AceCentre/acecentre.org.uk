...
export const request = async (url, query, params) => {
  ...
  if (parsedResult && parsedResult.redisEntryCreatedAt) {
    const currentTime = new Date();
    const createdAt = new Date(parsedResult.redisEntryCreatedAt);
    const elapsedTime = currentTime - createdAt;
    console.log({ value, parsedResult, currentTime, createdAt, elapsedTime });

    // If the redis input was created more than 2 minutes ago update it in the background
    // Although we still serve the stale data from redis
    if (elapsedTime > 120000) {
      console.log("Revalidate redis key: ", fullRedisKeyForLogging);
    }
  }
