...
export const request = async (url, query, params) => {
  ...
  const safeGraphqlRequest = async (url, query, params) => {
    try {
      ...
      return await graphqlRequest(url, query, params, headers);
    } catch (error) {
      console.error(`GraphQL request failed for ${queryName}:`, error);

      // If the error contains data (GraphQL returned both errors and data), return the data
      if (error?.response?.data) {
        console.log(
          `Returning partial data for ${queryName} despite GraphQL errors`
        );
      ...
      if (queryName.includes("products") || queryName.includes("Products")) {
        ...
        queryName.includes("categories") ||
        queryName.includes("Categories")
      ) {
        return { categories: { nodes: [] } };
      } else if (
        queryName.includes("projects") ||
        queryName.includes("Projects")
      ) {
        return { projects: { nodes: [] } };
      } else if (
        queryName.includes("caseStudies") ||
        queryName.includes("CaseStudies")
      ) {
        return { caseStudies: { nodes: [] } };
      } else if (
        queryName.includes("trustees") ||
        queryName.includes("Trustees")
      ) {
        ...
        queryName.includes("testimonials") ||
        queryName.includes("Testimonials")
      ) {
        return { testimonials: { nodes: [] } };
      } else if (
        queryName === "getCategoryBySlug" ||
        queryName?.includes("CategoryBySlug")
      ) {
        return { category: null };
      } else if (queryName.includes("page") || queryName.includes("Page")) {
        return { page: null };
      } else if (
        queryName.includes("refreshJwtAuthToken") ||
        queryName.includes("RefreshJwtAuthToken")
      ) {
        return { refreshJwtAuthToken: { authToken: null } };
      } else {
        // Generic fallback - return empty object with common GraphQL structure
        return {
          products: { nodes: [] },
          posts: { nodes: [] },
          staff: { nodes: [] },
