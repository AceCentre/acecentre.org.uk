...
export const getAllPostCards = async () => {
  try {
    ...
    console.error("GraphQL request failed for post cards:", error);
    return [];
  }
};
...
export const getAllFullPosts = async () => {
  try {
    ...
    console.error("GraphQL request failed for all full posts:", error);
...
export const getPostBySlug = async (slug) => {
  ...
  try {
    const { post } = await request(ENDPOINT, GetSinglePostBySlug, { slug });
    if (!post) return null;
    ...
    console.error("GraphQL request failed for post by slug:", error);
    return null;
  }
};
...
/**
 * @param {string} categorySlug - WordPress category slug (matches `where.categoryName` in WPGraphQL)
 * @param {string} [categoryTitle] - Display name for cards; defaults to slug
 */
export const getAllPostsForCategory = async (categorySlug, categoryTitle) => {
  if (!categorySlug) throw new Error("No category specified");
  const displayName = categoryTitle ?? categorySlug;
  ...
  try {
    ...
    return nodes.map((project) => {
      return {
        slug: project.slug,
        title: project.title,
        mainCategoryName: displayName,
        featuredImage: getFeaturedImage(project),
      };
    });
  } catch (error) {
    console.error("GraphQL request failed for posts by category:", error);
    // Return empty array to prevent build failure
    return [];
  }
};
...
export const getLandingPagePosts = async () => {
  try {
    ...
    console.error("GraphQL request failed for landing page posts:", error);
    return null;
  }
};
