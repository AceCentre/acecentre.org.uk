...
export const getLanguageLibraryLandingPageData = async () => {
  ...
  const helpfulLinksResponse = await fetch(
    "https://language-library.acecentre.org.uk/wp-json/language-library/v1/helpful-links"
  );

  const helpfulLinksResult = await helpfulLinksResponse.json();

  return {
    featuredPosts,
    languages: arrayUniqueByKey(allLanguages, "slug"),
    helpfulLinks: helpfulLinksResult,
  };
};
...
export const getFields = async () => {
  const response = await fetch(
    "https://language-library.acecentre.org.uk/wp-json/language-library/v1/fields"
  );
